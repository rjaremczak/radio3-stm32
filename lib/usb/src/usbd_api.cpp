...
/* The table maps the endpoint address to the internal endpoint index.
   The entries endPointRxIndex[0] and endPointTxIndex[0] are always zero. */
...
/* The number of configured internal endpoints including the default control endpoint */
...
UsbResult USBDendPointConfigure(uint8_t endPoint, usb_transfer_t type, uint16_t rxBuffSize, uint16_t txBuffSize) {
    /* Microcontroller supports only 8 endpoints. The standard allows up to 16 endpoints. */
    if (endPointCount >= EP_MAX_COUNT || endPoint >= USB_EP_MAX_COUNT) {
...
/* Copy data from a buffer in the user memory to the packet memory area (PMA).
    pma_offset - destination offset in the PMA
    buffer     - source pointer
    count      - the number of bytes to be copied */
static void LowLevelWrite(uint32_t pma_offset, uint8_t const *buffer, uint32_t count) {
    uint32_t i, n;
    uint16_t *pma;
...
/* Copy data from the packet memory area (PMA) to a buffer in user memory.
    ...
    count      - the number of bytes to be copied */
static void LowLevelRead(uint32_t pma_offset, uint8_t *buffer, uint32_t count) {
    uint32_t i, n;
    uint16_t const *pma;
    uint16_t *buffer16;
...
UsbResult USBDgetEndPointStatus(uint8_t epaddr, uint16_t *status) {
    uint16_t ep;
...
UsbResult USBDsetEndPointHalt(uint8_t epaddr) {
    uint16_t ep;
...
UsbResult USBDclearEndPointHalt(uint8_t epaddr) {
    uint16_t ep;
