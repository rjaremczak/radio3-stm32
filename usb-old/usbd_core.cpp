...
struct UsbDeviceState {
    uint32_t visibleState;
    UsbControlState controlState;
    UsbSetupPacket setup;
    UsbdCallbackList const *callback;
...
static UsbDeviceState DeviceState;

/** Device initialization **/

int USBDcoreConfigure() {
    memset(&DeviceState, 0, sizeof(UsbDeviceState));
...
void USBDreset(usb_speed_t speed) {
    memset(&DeviceState.setup, 0, sizeof(UsbSetupPacket));
...
static void Setup0(UsbDeviceState *);
static void NoDataSetup0(UsbDeviceState *);
static void InDataSetup0(UsbDeviceState *);
static void In0(UsbDeviceState *);
static void DataStageIn0(UsbDeviceState *);
static void OutDataSetup0(UsbDeviceState *);
static void Out0(UsbDeviceState *);
static void DataStageOut0(UsbDeviceState *);
...
void USBDtransfer(uint8_t ep, UsbPid token) {
...
void Setup0(UsbDeviceState *ds) {
    uint16_t len;

    len = USBDread0((uint8_t *) &(ds->setup), sizeof(UsbSetupPacket));

    if (ds->controlState != IDLE || len != sizeof(UsbSetupPacket)) {
...
void NoDataSetup0(UsbDeviceState *ds) {
    UsbResult result;
    uint8_t recipient;
...
void InDataSetup0(UsbDeviceState *ds) {
...
void DataStageIn0(UsbDeviceState *ds) {
...
void OutDataSetup0(UsbDeviceState *ds) {
    UsbResult result;
...
void DataStageOut0(UsbDeviceState *ds) {
    uint16_t length;
