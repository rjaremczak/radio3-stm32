...
enum { LOG_PROBE=0, LIN_PROBE=1, VNA=2 } AnalyserSource;
...
enum { ANALYSER_IDLE=0, ANALYSER_BUSY=1, ANALYSER_INVALID_REQUEST=2 } AnalyserState;
...
static void performAnalysis(struct AnalyserRequest *req) {
	...
	while(step <= numSteps) {
		...
		case LOG_PROBE: analyserData.data[step++] = adc_readLogarithmicProbe(); break;
		case LIN_PROBE: analyserData.data[step++] = adc_readLinearProbe(); break;
		case VNA:
...
static void handleIncomingFrame(void) {
	...
	case DEVICE_RESET: radio3_init(); break;
	case DEVICE_INFO: cmdGetDeviceInfo(); break;
	case DEVICE_STATE: cmdGetDeviceState(); break;

	case VFO_GET_FREQ: cmdGetVfoFrequency(); break;
	case VFO_SET_FREQ: cmdSetVfoFrequency(payload); break;

	case LOGPROBE_GET: cmdSampleLogarithmicProbe(); break;
	case LINPROBE_GET: cmdSampleLinearProbe(); break;
	case CMPPROBE_GET: cmdSampleComplexProbe(); break;
	case FMETER_GET: cmdSampleFMeter(); break;
	case PROBES_GET: cmdSampleProbes(); break;
	case PROBES_START_SAMPLING: deviceState.probesSampling = 1; break;
	case PROBES_STOP_SAMPLING: deviceState.probesSampling = 0; break;

	case ANALYSER_REQUEST: cmdAnalyserStart(payload); break;
...
static void handleDataSampling(void) {
	...
	if(currentTime > nextSamplingTimeMs) {
		...
		if(deviceState.probesSampling) { cmdSampleProbes(); waitMs(10); }
	}
}
